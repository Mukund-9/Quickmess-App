...
export const useSocketStore = create<SocketState>((set, get) => ({
  ...
  connect: (token, queryClient) => {
    ...
    // cleanup old socket instance if it exists
    ...
    socket.on("new-message", (message: Message) => {
      ...
      queryClient.setQueryData<Message[]>(["messages", message.chat], (old) => {
        ...
        // remove optimistic temp messages once a real one arrives
      ...
      // update chat lastMessage for instant UI update
  ...
  sendMessage: (chatId, text, currentUser) => {
    ...
    // if server emits socket-error, remove optimistic message
